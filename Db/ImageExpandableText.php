<?php
/**
 * InstaPress Cms_Db_ImageExpandableText
 * LICENSE
 * @author		Pramod Thakur
 * @package		InstaPress
 * @copyright	Copyright (c) Citizen Media Pvt. Ltd. (http://www.instablogs.com/)
 */
class Cms_Db_ImageExpandableText extends Cms_Db_Abstract
{
	protected $_mainTable = "cms_image_expandable_text";
	...
	function add()
	{
		...
		{
			throw new Exception(gettext("ImageExpandableText : ").gettext("clientId should be a natural number!"));
		}
			
		$imageId = isset($this->_arrayUpdatedData['imageId'])?trim($this->_arrayUpdatedData['imageId']):"";
		if( !$imageId || !is_numeric( $imageId ))
		{
			throw new Exception(gettext("ImageExpandableText : ").gettext("imageId should be a natural number!"));
		}
		...
		if( !$xElementId )
		{
			throw new Exception(gettext("ImageExpandableText : ").gettext("Expandable Name is a required parameter and cannot be blank!"));
		}
		
		$xElementValue = isset($this->_arrayUpdatedData['xElementValue'])?trim($this->_arrayUpdatedData['xElementValue']):"";
		if( !$xElementValue )
		{
			throw new Exception(gettext("ImageExpandableText : ").gettext("Expandable Value is a required parameter and cannot be blank!"));
		}
		...
		{
			throw new Exception(gettext("ImageExpandableText : ").gettext("createdUserId should be a natural number!"));
		}

		$objTempImage = new Cms_Db_ImageExpandableText();
		...
		if( $totalCount > 0 ) {
			throw new Exception(gettext("ImageExpandableText : ").gettext("This Expandable image data already exists!"));
		}
	...
	function edit()
    {
		...
		if( !$clientId || !is_numeric( $clientId ) )
		{
			throw new Exception(gettext("ImageExpandable : ").gettext("clientId shentn_image_expandableould be a natural number!"));
		}
