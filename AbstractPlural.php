<?php
	...
	abstract class Cms_AbstractPlural {
		...
		public function __call( $functionName, $argumentsArray ) {
			...
			try {
				if( count( $argumentsArray ) > 0 && is_numeric( $argumentsArray[ 0 ] ) && $argumentsArray[ 0 ] < count( $this->getResultCount() ) ) {
					$singular = $this ->get( $argumentsArray[ 0 ] );
					return $singular->$functionName();
				}
				$singular = $this ->get( 0 );
				return $singular->$functionName();
			} catch( Exception $ex ) {
