<?php
	 ...
	 */
	abstract class Cms_AbstractSingular {
		...
		public function __call( $functionName, $arguments ) {
			if( substr( $functionName, 0, 3 ) == 'get' ) {
				...
				if( isset( $this->_recordArray[ $varName ] ) ) {
					...
					if( $this->_selfDbObj === false ) {
						$dbClass = $this->_dbClassPrefix . $this->_dbClass;
						$selfDbObj = new $dbClass();
						$pkFunction = 'get' . ucfirst( $this->_foreignKey );
						$selfDbObj->set( "$this->_foreignKey||" . $this->$pkFunction()  );
						$this->_selfDbObj = $selfDbObj; 
					}
					return $this->_selfDbObj->$functionName( 0, $arguments[ 0 ] );
				} else {
