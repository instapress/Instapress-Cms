 ...
 *
 */
class Cms_Connection_Master {

	// Server Variables
	private $server   = CMS_MASTER_SERVER; //database server
	private $user     = CMS_MASTER_USER; //database login name
	private $pass     = CMS_MASTER_PSWD; //database login password
	private $database = CMS_MASTER_DB; //database name
	private $pre      = ""; //table prefix
	private $autoincrement_pre = CMS_MASTER_PREFIX;
	...
	function QueryUpdate($table, $data, $where='1') {
		...
		foreach($data as $key=>$val) {
			...
			elseif(strtolower($val)=='unix_timestamp()') $q.= "`$key` = UNIX_TIMESTAMP(), ";
